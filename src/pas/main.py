...
def render(result):
    ...
    elif result["output_type"] == "nested_single_dict":
        return result["result"]["content"]
    
    elif result["output_type"] == "single_list":
        return "\n".join(
...
def cli_level(lowered, agent):
    if lowered.startswith("see history"):
        ...
        which = splitted[-1]
        if which in ["episodic", "semantic"]:
            if which == "episodic":
                memory = agent.context.access_memory(which).episodes
                return {
                    "output_type": "single_list",
                    "success": True,
                    "result": memory
                ...
                memory = agent.context.access_memory(which).knowledge
                return {
                    "output_type": "single_dict",
                    "success": True,
                    "result": memory
    ...
    elif lowered.startswith("belief"):
        belief = agent.context.belief
        return {
            "output_type": "single_dict",
            "success": True,
            "result": belief
...
def main():
    ...
    while True:
        ...
        cli_res = cli_level(lowered, agent)
        ...
        with console.status("[blue]Agent >[/blue] ..."):
            result = agent.run_agent(lowered)
